...
export const v1: PromptVersion = {
    id: 'v1',
    name: 'Standard',
    description: 'Balanced technical and artistic control.',
    prompts: {
...
3. **Descriptive & Rich**: Use descriptive phrases and detailed imagery. Those full sentences are still discouraged to save tokens, DO NOT be overly terse. Expand on textures, lighting, and atmosphere.
   - **Subject & Environment**: Use evocative language. Instead of "man, street", use "weathered elderly man, bustling rain-slicked cyber-street".
4. Avoid repetition. Each field has a STRICT scope:
   - **spatialRelationship**: **Spatial Design & Logic**. Define precise relative positions. For environments, adhere to **Spatial Design Rules** (e.g., perspective flow, architectural logic, effective use of negative space). Ensure elements are physically coherent.
   - **composition**: **Cinematic Framing**. Use professional techniques (e.g., "Rule of Thirds", "Dutch Angle", "Symmetry") to reinforce the mood. Choose angles that heighten the emotional impact (e.g., "low angle for dominance", "overhead for vulnerability").
   - **visualLayers**: **Expertly Design Depth**. Don't just list objects. Create a visual journey: **Foreground** (framing/leading elements), **Middle-ground** (subject interaction), **Background** (context/atmosphere). Ensure layers serve the narrative intent.
...
6. **Balanced Description**: Maintain objective physical descriptions but infuse them with appropriate atmospheric adjectives.
   - *Example*: Instead of just "blue light", say "melancholic cold blue aesthetic lighting".
...
2. Enhance details, lighting, and style using descriptive adjectives.
